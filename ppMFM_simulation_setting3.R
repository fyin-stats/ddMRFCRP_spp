...
pp_region_list <- readRDS("./pp_region_list_setting3.rds")
true_cluster_membership_setting <- readRDS("./true_cluster_membership_setting3.rds")
##########
nsim <- 100
###########
time1 <- Sys.time()
registerDoParallel(cores=50)
rlt_setting3 <- foreach(ii = 1:nsim, .errorhandling="pass") %dopar%{
  ...
  result <- list(K_hat = max(z_fitted), ri = ri, adj_ri = adj_ri, samples = temp_result,
                 ...
                 ri_trace = ri_trace, adj_ri_trace = adj_ri_trace, est_lambda = matrix(temp_Dahl_result$phiout[temp_Dahl_result$zout], 
...
for(ii in 1:nsim){
  ...
  est_lambda[,,ii] <- rlt_setting3[[ii]]$est_lambda
...
######################
saveRDS(simulation_results, "MFM_simulation_results_setting3.rds")