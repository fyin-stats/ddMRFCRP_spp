...
packages <- c("spatstat","doParallel","MHadaptive","purrr","dplyr","fossil")
ipak(packages)
# library(spatstat)
# library(doParallel)
####################
...
###################
### setting 2: five clusters
### (0.2,1,5,20,80)
### 8*10 : 4
### 4*20 : 1
grid <- c(20,20)
# eta_vec <- seq(0,5,by=0.25)
...
nsim <- 100
pp_region_list <- readRDS("./pp_region_list_setting2.rds")
true_cluster_membership_setting <- readRDS("./true_cluster_membership_setting2.rds")
#
registerDoParallel(cores=50)
###############
time1 <- Sys.time()
rlt_setting2 <- foreach(ii = 1:nsim, .errorhandling="pass") %dopar%{
...
# ri_trace_df <- data.frame(iter = NA, )
...
######################
saveRDS(simulation_results, "simulation_results_setting2.rds")